...
import argparse
import re
from telestai.blockchain import Blockchain
from telestai.core import b2x, b2lx, COIN
from telestai.wallet import CTelestaiAddress, CTelestaiAddressError
from telestai.core.script import OP_RETURN

parser = argparse.ArgumentParser(
    description='Scans Telestai block database files (blk*.dat) for text in OP_RETURNs')
parser.add_argument('-D', '--dir', default="~/.telestai/blocks/",
                    type=str, help="Directory containing Telestai block database")
...
parser.add_argument('-E', '--endblock', default=0, type=int,
                    help="Scan ending block (default: all)")
parser.add_argument('--cachefile', default="", type=str,
                    help="Block database index cache filename (default: no caching)")
parser.add_argument('--extendedcharsets', action="store_true",
                    help="Try more character sets (default: ascii)")
...
print("Scanning blocks {}-{}".format(args.startblock,
      endblock if endblock is not None else "all"))
