"""
`FakeTelestaiProxy` allows for unit testing of code that normally uses telestai
RPC without requiring a running telestai node.

`FakeTelestaiProxy` has an interface similar to `telestai.rpc.Proxy`, but does not
connect to a local telestai RPC node. Hence, `FakeTelestaiProxy` is similar to a
mock for the RPC tests.

`FakeTelestaiProxy` does _not_ implement a full telestai RPC node. Instead, it
currently implements only a subset of the available RPC commands. Test setup is
responsible for populating a `FakeTelestaiProxy` object with reasonable mock
data.
...
from telestai.core import (
    ...
    # number of satoshis per telestai
    COIN,
...
from telestai.wallet import (
    # telestai address initialized from base58-encoded string
    CTelestaiAddress,
...
def make_address_from_passphrase(passphrase, compressed=True, as_str=True):
    """
    Create a Telestai address from a passphrase. The passphrase is hashed and
    then used as the secret bytes to construct the CTelestaiSecret.
    """
    if not isinstance(passphrase, bytes):
        passphrase = bytes(passphrase, "utf-8")
    passphrasehash = hashlib.sha256(passphrase).digest()
    private_key = CTelestaiSecret.from_secret_bytes(
        passphrasehash, compressed=compressed)
    address = P2PKHTelestaiAddress.from_pubkey(private_key.pub)
    if as_str:
        return str(address)
    else:
        return address
...
def make_txout(amount=None, address=None, counter=None):
    ...
    txout = CMutableTxOut(amount, CTelestaiAddress(address).to_scriptPubKey())
...
def make_blocks_from_blockhashes(blockhashes):
    """
    Create some block data suitable for FakeTelestaiProxy to consume during
    instantiation.
    """
    blocks = []
...
class FakeTelestaiProxyException(Exception):
    """
...
class FakeTelestaiProxy(object):
    """
    This is an alternative to using `telestai.rpc.Proxy` in tests. This class
    ...
    def _call(self, rpc_method_name, *args, **kwargs):
        ...
        python-telestailib does not parse.
        """
    ...
    def getblock(self, blockhash, *args, **kwargs):
        ...
        # Note that the actual "getblock" telestaid RPC call from
        # python-telestailib returns a CBlock object, not a dictionary.
    ...
    def getnewaddress(self):
        ...
        return CTelestaiAddress(address)
    ...
    def fundrawtransaction(self, given_transaction, *args, **kwargs):
        ...
        if isinstance(given_transaction, str):
            ...
            raise FakeTelestaiProxyException(
    ...
    def _batch(self, batch_request_entries):
        ...
        feature found in python-telestailib and telestaid RPC.
        """
        ...
        for (idx, request) in enumerate(batch_request_entries):
            ...
            if isinstance(request["params"], list):
                ...
                # matches error message received through python-telestairpc
