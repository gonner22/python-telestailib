import unittest
from telestai.core.key import CPubKey, CECKey
import os
import hashlib

from telestai.core.script import OP_CHECKMULTISIG, CScript, CreateMultisigRedeemScript
from telestai.crypto import verify_multisig_script, verify_signature
from telestai.wallet import P2PKHTelestaiAddress, P2SHTelestaiAddress, CTelestaiSecret
from telestai.core.transaction import CMultiSigTransaction
...
class TestP2SHFunctions(unittest.TestCase):
    ...
    def test_verify_signature(self):
        ...
        print("private key wif: ", CTelestaiSecret.from_secret_bytes(private_key_bytes).to_wif())
        # print(f"Signature length: {len(sig)}")
        public_key_bytes = bytes.fromhex(pubkey.hex())
        print("public key bytes: ", public_key_bytes)
        p2pkh_address = P2PKHTelestaiAddress.from_pubkey(public_key_bytes)
    ...
    def test_create_and_verify_p2sh_address(self):
        try:
            ...
            p2sh_address = P2SHTelestaiAddress.from_redeemScript(redeem_script)
            print("P2SH Address Generated", p2sh_address)

            # Verify the P2SH address is an instance of P2SHTelestaiAddress
            self.assertIsInstance(p2sh_address, P2SHTelestaiAddress)
