...
class CECKey:
    ...
    def sign(self, hash): # pylint: disable=redefined-builtin
        ...
        assert 1 == result
        if telestai.core.script.IsLowDERSignature(mb_sig.raw[:sig_size0.value]):
            return mb_sig.raw[:sig_size0.value]
        else:
    ...
    def sign_compact(self, hash): # pylint: disable=redefined-builtin
        ...
        sig = telestai.signature.DERSignature.deserialize(sig)
        ...
        # telestai core does <4, but I've seen other places do <2 and I've never seen a i > 1 so far
        for i in range(0, 4):
            cec_key = CECKey()
