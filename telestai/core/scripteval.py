...
unlikely to match Satoshi Telestai exactly. Think carefully before using this
module.
"""

from __future__ import absolute_import, division, print_function, unicode_literals
from telestai.core.script import *
import telestai.core.serialize
...
class EvalScriptError(telestai.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = telestai.core._bignum.vch2bn(s)
...
def _CheckMultiSig(opcode, script, stack, txTo, inIdx, flags, err_raiser, nOpCount):
    ...
    # Note how Telestai Core duplicates the len(stack) check, rather than
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(telestai.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(telestai.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(telestai.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(telestai.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(telestai.core.ValidationError):
    pass
...
class VerifySignatureError(telestai.core.ValidationError):
    pass
