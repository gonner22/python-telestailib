...
class CTransaction(ImmutableSerializable):
    ...
    def stream_deserialize(cls, f):
        ...
        This implementation corresponds to Telestai's SerializeTransaction() and
        consensus behavior. Note that Telestai's DecodeHexTx() also has the
...
class CBlock(CBlockHeader):
    ...
    def build_merkle_tree_from_txids(txids):
        ...
        duplicate txids, resulting in a vulnerability. (CVE-2012-2459) Telestai
...
class CoreChainParams(object):
    """Define consensus-critical parameters of a given instance of the Telestai system"""
    MAX_MONEY = None
    GENESIS_BLOCK = None
...
class CoreMainParams(CoreChainParams):
    MAX_MONEY = 1969194240 * 1000 * COIN
    NAME = 'mainnet'
    ...
    SUBSIDY_HALVING_INTERVAL = 210240 * 10
...
def _SelectCoreParams(name):
    ...
    Don't use this directly, use telestai.SelectParams() instead so both
