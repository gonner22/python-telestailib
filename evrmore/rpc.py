...
def get_evr_datadir(datadir=None):
    if datadir is None:
    ...
    def __init__(self,
        ...
        if service_url is None:
            ...
            if service_port is None:
                service_port = evrmore.params.RPC_PORT
            ...
            if evrmore.params.NAME != "mainnet":
                cookie_dir = os.path.join(cookie_dir, evrmore.params.NAME)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a evrmore JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function evrmore.rpc.name> rather than <function
        # evrmore.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a evrmore RPC service
    ...
    def __init__(self,
        ...
        Usually no arguments to ``Proxy()`` are needed; the local evrmored will
        be used.
    ...
    def generate(self, numblocks):
        """
        DEPRECATED (will be removed in evrmore-core v0.19)
    ...
    def getblock(self, block_hash):
        ...
        try:
            ...
            raise TypeError('%s.getblock(): block_hash must be bytes; got %r instance' %
                            (self.__class__.__name__, block_hash.__class__))
        try:
            # With this change ( https://github.com/evrmore/evrmore/commit/96c850c20913b191cff9f66fedbb68812b1a41ea#diff-a0c8f511d90e83aa9b5857e819ced344 ),
            # evrmore core's rpc takes 0/1/2 instead of true/false as the 2nd argument which specifies verbosity, since v0.15.0.
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by evrmored. See
        https://en.evrmore.it/wiki/BIP_0022 for full specification.
        """
...
class RavenRawProxy(RawProxy):
    def __init__(self,
        ...
        if service_url is None:
            if evr_conf_file is None:
                evr_conf_file = get_evr_conf(datadir)

        super(RavenRawProxy, self).__init__(service_url=service_url,
                                            service_port=service_port,
                                            btc_conf_file=evr_conf_file,
                                            timeout=timeout)
...
class RavenProxy(Proxy):
    ...
    def transferfromaddress(self, asset_name, from_address, qty, to_address, message="",
                            expire_time=0, evr_change_address="", asset_change_address=""):
        check_numeric(qty)
        r = self._call('transferfromaddress', str(asset_name), str(from_address), qty, str(to_address),
                       str(message), int(expire_time), str(evr_change_address), str(asset_change_address))
        return r
    ...
    def transferfromaddresses(self, asset_name, from_addresses, qty, to_address, message="",
        ...
        r = self._call('transferfromaddresses', str(asset_name), str(from_addresses), qty, str(to_address),
                       str(message),
                       int(expire_time), str(evr_change_address), str(asset_change_address))
        return r
    ...
    def subscribetochannel(self, channel_name):
        ...
        # (see CMessageChannel class in evrmore.messaging)
